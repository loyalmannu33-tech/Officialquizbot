...
def _build_system_prompt(count: int, difficulty: str, language: str) -> str:
    ...
    is_bilingual = any(k in lang_lower for k in (
        "bilingual", "hindi+english", "english+hindi", "en/hi", "hi/en",
        "hinglish", "both", "dual",
    )) or lang_lower in ("hindi", "english")
    if is_bilingual:
        lang_rule = (
            "LANGUAGE — STRICT BILINGUAL (English + Hindi) for EVERY question, option, and explanation.\n"
            "Format key terms and full questions in BOTH languages side-by-side using a slash, e.g.:\n"
            '• Question: "What is a cell? / कोशिका क्या है?"\n'
            '• Option: "Cell / कोशिका" or "Nucleus / केंद्रक"\n'
            "• Explanation: short bilingual line the same way.\n"
            "Never output only English or only Hindi. Both languages must appear in the question, "
            "in every option, and in the explanation. Proper nouns may stay as commonly written.\n\n"
        )
    else:
        lang_rule = (
            f"LANGUAGE — every single piece of text you output — the question, all 4 options, "
            f"AND the explanation, for EVERY item — must be written ENTIRELY in {language}. "
            f"Translate if the source is in another language. Never mix languages unless "
            f"{language} itself is a mix like Hinglish. Proper nouns may stay as commonly written.\n\n"
        )
    return (
...
async def ask_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌐 Bilingual EN/HI", callback_data="lang_Bilingual EN/HI")],
        [InlineKeyboardButton("✍️ Type another language", callback_data="lang_other")],
    ])
    await context.bot.send_message(
        chat_id,
        "Pick a language:\n"
        "_Bilingual = questions like_ `What is a cell? / कोशिका क्या है?` _and options_ `Cell / कोशिका`",
        parse_mode="Markdown",
...
async def ask_ai_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        "(recommended 15–30s). DM me timer nahi lagta — polls seedha bhej diye jaate hain:",
        reply_markup=keyboard,
    )
    return ASK_AI_TIMER
...
async def oq_receive_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Step 2/3 — pick a per-question timer (15–30s recommended). Group me ye timer lagega; DM me polls bina timer ke post honge:", reply_markup=keyboard)
    return OQ_TIMER
...
async def _finish_live_quiz(bot, chat_id, application):
    ...
    if ranking:
        ...
        header = (
            ...
            f"_{FONT_STYLE}_\n"
            f"👥 {len(ranking)} participant(s) — sab dikhega (IDs included)\n\n"
        )
        chunks, current = [], header
        for i, s in enumerate(ranking):
            ...
            if len(current) + len(line) > 3800:
                ...
                current += line
        current += "\n👤 User ID is shown so the host can reach out directly (prizes, verification, etc.)."
        chunks.append(current)
        for chunk in chunks:
            try:
